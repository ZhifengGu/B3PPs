...
def select(x):
    ...
    with open('MergeIG.csv') as a_file, open(path_b) as b_file:
        ...
        path_c = '.\\models\\'+str(x)+'\\feature.csv'
...
def testing_read():
    ...
    test_fasta = open(".\\peptide_sequences.txt", 'w+')
...
def get_merge():
    f_AAC = AAC('./peptide_sequences.txt')
    f_AAC.AAC()
    f_AAC.save('AAC.csv')

    f_CKSAAP = CKSAAP('./peptide_sequences.txt')
    f_CKSAAP.CKSAAP()
    f_CKSAAP.save('CKSAAP.csv')

    f_DDE = DDE('./peptide_sequences.txt')
    f_DDE.DDE()
    f_DDE.save('DDE.csv')

    f_APAAC = APAAC('./peptide_sequences.txt')
    f_APAAC.APAAC()
    f_APAAC.save('APAAC.csv')

    f_ASDC = ASDC('./peptide_sequences.txt')
    f_ASDC.ASDC()
    f_ASDC.save('ASDC.csv')

    f_CTDC = CTDC('./peptide_sequences.txt')
    f_CTDC.CTDC()
    f_CTDC.save('CTDC.csv')

    f_CTDT = CTDT('./peptide_sequences.txt')
    f_CTDT.CTDT()
    f_CTDT.save('CTDT.csv')

    f_CTDD = CTDD('./peptide_sequences.txt')
    f_CTDD.CTDD()
    f_CTDD.save('CTDD.csv')

    f_QSO = QSO('./peptide_sequences.txt')
    f_QSO.QSO()
    f_QSO.save('QSO.csv')
...
def get_selection():
    data = Analysis()
    data.data_import('.\\Merge.csv')
    data.frame.insert(0, 'Labels', data.label)
    data.frame.to_csv('.\\MergeIG.csv', sep=',', header=True, index=False)
    
    i = 1
    select(i)
    
    os.remove('./Merge.csv')
    os.remove('./MergeIG.csv')

def get_result(fasta):
    i = 1
    fasta.x = i
    test_dir = ".\\models\\" + str(i) + "\\feature.csv"
...
def calculate():
    ...
    os.remove(".\\peptide_sequences.txt")
